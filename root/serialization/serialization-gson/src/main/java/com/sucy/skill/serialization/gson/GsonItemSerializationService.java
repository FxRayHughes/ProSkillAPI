...
public final class GsonItemSerializationService implements ItemSerializationService {
    ...
    /**
     * 读取上一次兼容性重构留下的 Bukkit 对象流格式，使升级中的服务器不丢失战斗快照。
     *
     * <p>写入早已改走 Gson，这里是只读的迁移路径。{@code BukkitObjectInputStream}
     * 自 1.21 起标记为弃用，但它仍是解析既有存档的唯一手段——新 API
     * （{@code ItemStack.deserializeBytes}）读不了旧的对象流字节。因此保留调用，
     * 并用 try-with-resources 确保流被关闭（原实现在异常路径会泄漏）。</p>
     */
