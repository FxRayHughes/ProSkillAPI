 ...
 * 1.20 世代的桥接实现。
 *
 * <p>1.20 起 ItemMeta 可直接清空属性修饰符，比遮蔽 HIDE_ATTRIBUTES 更彻底；
 * 该接口不可用时仍回落到旧的遮蔽方式。</p>
 *
 * <p>本类刻意不继承其他版本模块：每个世代自带完整实现，
 * 改动一个版本不会静默影响另一个。</p>
 */
public class V1_20Bridge implements NmsBridge {
     ...
     * 清空物品的属性修饰符。1.20 之前没有这个公开接口，


     * 因此查不到方法时由调用方回落到 HIDE_ATTRIBUTES 遮蔽。


     */


    private boolean clearModifiers(ItemMeta meta) {
        ...
        try {
            ...
            SET_ATTRIBUTE_MODIFIERS.invoke(meta, com.google.common.collect.ImmutableMultimap.of());
