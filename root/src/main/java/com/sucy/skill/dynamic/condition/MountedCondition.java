...
@SkillNode(
        ...
        descriptionZh = "检查目标身上是否骑着别的实体（目标是被骑的一方）。必须至少有一个乘客；类型列表留空或包含“Any”时任意乘客都算通过，否则乘客类型需与列表有交集。",
        container = true)
public class MountedCondition extends ConditionComponent {
    ...
    @SkillField(
            ...
            label = "Types",
            labelZh = "类型列表",
            tooltip = "[types] The entity types that can be mounting the target",
            tooltipZh = "允许的乘客实体类型列表。留空或含“Any”表示不限类型；名称转大写并把空格替换为下划线后与 EntityType 枚举名比对。",
