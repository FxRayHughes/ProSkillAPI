...
@SkillNode(
        ...
        descriptionZh = "玩家破坏方块时触发，施法者与初始目标都是这名玩家（与被破坏的方块无关，方块信息只以数值形式传下去）。已被领地/保护插件取消的破坏不会触发。触发时把方块类型写入 api-block-type、方块坐标写入 api-block-loc，后续节点可用 Location 类目标或数值条件取用。",
        container = true)
public class BlockBreakTrigger implements Trigger<BlockBreakEvent> {
    @SkillField(
            ...
            label = "Material",
            labelZh = "材质",
            tooltip = "[material] The type of block expected to be broken",
            tooltipZh = "限定哪些方块才触发。留空或包含 Any 表示不限；否则把填写值里的空格换成下划线后与方块真实类型名做忽略大小写比较（如 Oak Log 匹配 OAK_LOG）。注意通配只认首字母大写的 Any，写成 any 会被当成方块名去匹配、结果永不触发。",
